...
wal=0
def oakydoky(subject):
    try:
        if subject.blockType=="wood":
            for item in items:
                if items.index(item)==wal:
                    item.texture="oakplanks"

            subject.blockType="oak_planks"
            hotspots[wal].item.blockType="oak_planks"
            hotspots[wal].stack=int(hotspots[wal].stack)*4
            hotspots[wal].t.text=hotspots[wal].stack
            hotspots[wal].item.update_stack_text()
    except:print("Du, des hod ned glappt!")
def craftydrafty(subject):
    try:
        if subject.blockType=="oak_planks":
            for item in items:
                if items.index(item)==wal:
                    item.texture="craftingtable"

            subject.blockType="crafting_table"
            hotspots[wal].item.blockType="crafting_table"
            hotspots[wal].stack=int(floor(int(hotspots[wal].stack)/4))
            hotspots[wal].t.text=hotspots[wal].stack
            hotspots[wal].item.update_stack_text()
    except:print("Du, des hod ned glappt!")
epO1=Button(text="Oak Planks",on_click=lambda:oakydoky(subject=SUB),scale=0.1,scale_x=0.2,visible=False)
epO2=Button(text="Crafting Table",on_click=lambda:craftydrafty(subject=SUB),scale=0.1,scale_x=0.2,y=0.1,visible=False)
def vis():
    epO1.visible = True
    epO1.enabled = True
    epO2.visible = True
    ...
    epO.visible=False
    epO.enabled=False
epO=Button(text="Craft",on_click=vis,scale=0.1,visible=False)
epO.enabled=False
epO1.enabled=False
epO2.enabled=False
#ff=1
def inv_input(key,subject,mouse):
    ...
    global SUB,wal#,ff
    SUB=subject
    wai.text=f'<black><bold>Position: {floor(subject.x)},{floor(subject.y)},{floor(subject.z)}'
    try:
        wnum = int(key)
        if wnum > 0 and wnum < 10:
            ...
            for h in hotspots:
                #ieee+=1
                h.color=color.white
                #if h.item.blockType=="oak_planks":
                #    ff=ieee
